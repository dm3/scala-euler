...
object Util {
  ...
  def readLinesAsNumbers(file: String)(separator: String): Iterator[List[Int]] =
    readLines(file)(_.split(separator).toList.map(_.toInt))

  def readLines[A](file: String)(transform: String => List[A]): Iterator[List[A]] =
    fromFile(file).getLines.map(_.stripLineEnd).map(transform)
  ...
  */
  ...
  def sumOfDivisors(n: Int) = (2 to Math.sqrt(n)).map(x => if (n % x == 0 && n / x != x) x + n / x else if (n % x == 0) x else 0).foldLeft(1)(_ + _)
  }
